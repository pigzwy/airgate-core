...
路线文档（逐步推进）。目的：把 airgate OpsPage 从当前 ~20% 覆盖度，逐步补齐到 sub2api 的完整度。
...
- [ ] **M1 系统资源监控（6 卡）** ⭐ 推荐先做，纯 Core 自采
- [ ] **M2 时间范围切换 + 平台/分组维度下钻** ⭐ 前端+查询扩展，不卡插件
- [ ] **M3 告警闭环（规则+事件+静音+邮件）** ⭐ Core 自有定时评估
- [ ] **M4 健康分数 + 诊断报告**（依赖 M1 的系统资源 + 现有错误率）
- [ ] **M5 WebSocket 实时推送**（架构改造，把 15s 轮询升级为推送）
- [ ] **M6 请求详情钻取**（成功/慢请求列表，当前只能看错误）

### P1 — 运维必备

- [ ] **M7 并发统计卡（平台/分组/账号/用户四维）** Core 自采（scheduler）
- [ ] **M8 TTFT + 延迟全分位（P50/P90/P99/Max）** Core 聚合（扩聚合器 + 插件上报 first_token，已有）
- [ ] **M9 延迟直方图（bucket 分布）** Core 聚合
- [ ] **M10 错误分布饼图（按错误码分类）** Core 聚合
- [ ] **M11 系统日志表 + 运行时日志级别调整** Core 自采（slog sink）
- [ ] **M12 错误日志增强 + 请求/上游错误拆分 + trace** 需扩插件上报

### P2 — 增强

- [ ] **M13 切换率趋势** Core 聚合（scheduler 事件）
- [ ] **M14 OpenAI Token 统计（按 model）** Core 聚合
- [ ] **M15 设置中心（阈值染色/保留策略/邮件报告/错误过滤开关）**
- [ ] **M16 吞吐图缩放 / URL 深链 / 全屏大屏 / 自动刷新可调**
