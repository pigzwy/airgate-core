...
const METRIC_CARD_CLASS = 'min-h-[64px] 2xl:min-h-[68px]';  // 缩小：72→64, 78→68
const METRIC_CONTENT_CLASS = 'flex min-w-0 flex-1 flex-row items-center justify-between gap-2.5 p-2.5 text-left 2xl:p-3';  // 缩小：gap-3→2.5, p-3→2.5, p-3.5→3
      ...
      <Card.Content className={METRIC_CONTENT_CLASS}>
        <div className={METRIC_COPY_CLASS}>
          ...
          <div className="mt-1 flex min-w-0 items-baseline gap-2">
            <div className="flex min-w-0 items-baseline font-mono text-lg font-semibold leading-none text-foreground 2xl:text-xl">
              {value}
              {valueSuffix ? <span className="ml-1.5 text-xs font-medium text-muted">{valueSuffix}</span> : null}
            </div>
            <div className={`min-w-0 truncate text-xs font-semibold ${META_TONE_CLASSES[metaTone]}`}>{meta}</div>
          </div>
        </div>
        {/* 图标徽章：统一金色（pigcoder 设计：金色单一 accent，不用彩虹色调）
            移动端显示（单列布局有空间），中等屏隐藏（多列紧凑），2xl 再显示 */}
        <span className="flex md:hidden 2xl:flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--field-radius)] bg-gold-subtle text-gold ring-1 ring-gold/20 shadow-sm">
          {icon}
        </span>
      </Card.Content>
    </Card>
      ...
      {Array.from({ length: 8 }).map((_, index) => (
        <Card className={METRIC_CARD_CLASS} key={index}>
          <Card.Content className={METRIC_CONTENT_CLASS}>
            ...
            <Skeleton className="flex md:hidden 2xl:flex h-9 w-9 shrink-0 rounded-[var(--field-radius)]" />
          </Card.Content>
        </Card>
      ))}
    </div>
...
function ModelDistributionCard({ trend }: { trend: DashboardTrendResp }) {
  ...
  const distributionTabs = (
    <Tabs selectedKey={tab} onSelectionChange={(key) => setTab(key as 'model' | 'user')}>
      <Tabs.List>
        <Tabs.Tab id="model" className="whitespace-nowrap">
          <Tabs.Indicator />
          <span>{t('dashboard.model_distribution')}</span>
        </Tabs.Tab>
        <Tabs.Tab id="user" className="whitespace-nowrap">
          <Tabs.Separator />
          <Tabs.Indicator />
          ...
          <Tabs selectedKey={range} onSelectionChange={(key) => setRange(key as RangePreset)}>
            <Tabs.List>
              {RANGE_PRESETS.map((item, index) => (
                <Tabs.Tab id={item} key={item} className="whitespace-nowrap">
